...
{
    //Normal atk
    public class Hero2Atk : SkillCore
    {
        //public Hero2 Hero;
        Vector3 Rot;
        public int SlotCombo;
        #region Initialize 
        ...
        public override void Awake()
        {
            ...
            if (GameSystem.Settings.SoundEnable) {
                SoundClip = new AudioClip[6];
                for (int i = 0; i < SoundClip.Length; i++)
                    SoundClip[i] = Resources.Load<AudioClip> ("Audio/Skill/H2Nor" + (i + 1).ToString ());
            }
        }
        //
        public override void Start()
        {
            ...
            SetupEffectExtension("H2Atk" + SlotCombo.ToString() + "Slash");//Khởi tạo hiệu ứng effect riêng cho từng skill của hero (nếu có)
        }
        //Sau khi được active
        private void OnEnable()
        {
            ...
            StartCoroutine(AutoDisCol(0.3f, gameObject));//Tự động tắt va chạm
            if (Team.Equals(1))
            {
