...
public class WorldMap : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Objects[16].transform.position = Objects[12].transform.position = Objects[13].transform.position;
    ...
    private void SetupTextLanguage()
    {
        for (int i = 0; i < 7; i++) //Tên vùng đất
            TextLanguage[i].text = Languages.lang[168 + i] + " Lv." + DataUserController.User.LevelMap[i];
    ...
    public void ButtonFunctions(int type)
    {
        switch (type)
        {
            ...
            case 3: //Chọn mode preview
                Module.BattleModeSelected = 0;
                Objects[16].transform.position = Objects[12].transform.position = Objects[13].transform.position; //Vị trí hiệu ứng lựa chọn
                TextLanguage[14].text = Languages.lang[255]; //Text hướng dẫn
                break;
            case 4: //Chọn mode random
                Module.BattleModeSelected = 1;
                Objects[16].transform.position = Objects[12].transform.position = Objects[14].transform.position; //Vị trí hiệu ứng lựa chọn
    ...
    private IEnumerator ActionHireAssassin()
    {
        ...
        if (GameSystem.ConfirmBoxResult == 1)
        {
            ...
            {
                UserSystem.DecreaseGems((float)Module.GemsForHireAssassin, false);
                BattleCore.ChangeDifficult(1, Module.WorldMapRegionSelected); //Giảm độ khó của map hiện tại 5 level
                ...
                //Set tên các vùng đất + level
                for (int i = 0; i < 7; i++) //Tên vùng đất
                    TextLanguage[i].text = Languages.lang[168 + i] + " Lv." + DataUserController.User.LevelMap[i];
                //Set lại tên map và level
                TextLanguage[7].text = TextLanguage[Module.WorldMapRegionSelected].text; //Set tên vùng lựa chọn
            }
        }
    }
}